...
class Table extends Component {
	...
	handleKeyDown2(e, rowIndex, columnIndex){
		return
	}

	move(key){
		let change_pos = this.keymap[key]
		if(change_pos){
			let moved = this.change_cell(change_pos)
			if(!moved){
				return
			}
		}
	...
	onCellExit(old_cell, new_cell){
		return
	}

	onCellEnter(new_cell){
		return
	}

	change_cell(x,y){
		let nx = this.state.current_position[0] + x
		let ny = this.state.current_position[1] + y
		if(
			nx < this.grid[0].length &&
			ny < this.grid.length &&
			nx >= 0 &&
			ny >= 0){
			this.setState({current_position: [nx, ny]})
			return true
		} else {
			return false
		}
